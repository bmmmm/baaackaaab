...
heartbeat/ntfy/webhook request construction and monitor-URL redaction, and
...
GitHub query, the HTTP header probe, and the heartbeat/ntfy/webhook network sends
touch the network, so they are not unit-tested — all degrade to nil/baseline (or,
for outbound monitoring, a logged no-op that never changes the run's exit code)
by construction.
